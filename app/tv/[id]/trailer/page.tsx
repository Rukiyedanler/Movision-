...
export default function TvTrailerPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    async function loadTrailer() {
      try {
        ...
        if (tv.trailerUrl) {
          setTrailerUrl(`https://www.youtube.com/embed/${tv.trailerUrl}?autoplay=1`)
        } else {
          setError("Bu dizi için fragman bulunamadı.")
        }
      } catch (err) {
